		...
		<div class="row">
			...
			<div class="col-sm-6">
				<div class="float-right">
					<a href="<?php echo site_url('product/downloadCSV');?>"><button type="button" id="create_excel" class="btn btn-primary" title="Download Complete List"><i class="far fa-file-excel"></i></button></a>&nbsp; 
			...
			<div class="row">														
				...
				<div class="col-md-2 col-sm-6">						
					<div class="form-group">
						...
						<?php $financialYear = $this->input->get('financialYear'); ?>
						<?php 
						echo form_dropdown('financialYear',$financialDateArray, $financialYear, 'class="form-control" id="financialYear"'); ?>
					</div>
				</div>
			...
			<div class='row'>
				<div class="col-sm-12 float-right">
					<div class="form-group float-right">
						<button type="button" id="clearBtn" class="btn-sm btn-danger ">Clear Search</button>
						<button type="button" id="button-filter" class="btn-sm btn-primary ">Search</button>					
					</div>
					<br>
				</div>
		...
		<style>
			...
			<thead>
				<tr>
				  ...
				  <th>HSN</th>
				  <th style='min-width:100px;' <?php if($this->input->get('order') && ($this->input->get('sort') == 'total_sold')){ echo 'class=sort_'.$this->input->get('order'); } else { echo ''; } ?> ><a href="<?php echo $total_sold_sort; ?>">Ordered Qty.</th>
				  <th>MRP</th>
				  <th>Action</th>
				</tr>
			</thead>
				...
				<?php if(($products)){ ?>
					<?php foreach($products as $product){ ?>				
						<tr>
							...
							<td><?php 
									if(!empty($product['total_sold'])){
									?>
										<a target='_blank' href="<?php echo site_url('orderList'); ?>?model=<?php echo $product['product_id']; ?>&country_id=<?php echo $country_id; ?>"><?php echo $product['total_sold']; ?></a>
									<?
									}else{
							...
							<td class="">
							    <a href="<?php echo site_url('productView'); ?>/<?php echo $product['product_id']; ?>"><i class="fas fa-eye m-r-5"></i></a>
							    ...
							    <a href="<?php echo site_url('editProduct'); ?>/<?php echo $product['product_id']; ?>" data-toggle="tooltip" data-placement="top" ><i class="far fa-edit m-r-5"></i></a>
							    <?php } ?>
							  </td>							
						</tr>
					<?php } ?>  
...
<script>
	$(document).ready(function(){
		...
		$("#clearBtn").click(function(){ 
			location.href='<?php echo site_url('product'); ?>';			
		});	
