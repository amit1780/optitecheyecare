...
class Stock_model extends CI_Model {
	...
	public function getStockList($limit, $start,$data) {
	   ...
	   /*  if(!empty($data['date_from'])){		
		...
		$this->db->select("*, SUM(qty) as qty, batch.mfg_date as s_mfg_date, batch.exp_date as s_exp_date,(SELECT SUM(qty) FROM svi_stock ss WHERE ss.batch_id = svi_stock.batch_id AND ss.store_id = svi_stock.store_id AND ss.status = 'A') AS totalqty, (SELECT SUM(qty) FROM svi_challan_product cp LEFT JOIN svi_challan sc ON (cp.challan_id=sc.challan_id) WHERE sc.is_deleted='N' AND cp.batch_id = svi_stock.batch_id AND cp.store_id = svi_stock.store_id $date_from) as challan_qty, (SELECT SUM(return_qty) FROM svi_challan_product cp LEFT JOIN svi_challan sc ON (cp.challan_id=sc.challan_id) WHERE sc.is_deleted='N' AND cp.batch_id = svi_stock.batch_id AND cp.store_id = svi_stock.store_id $date_from) as return_qty ,(SELECT(IFNULL(totalqty,0)+IFNULL(return_qty,0)-IFNULL(challan_qty,0))) AS availableQty")
            ->from('stock');
			...
			if(empty($data['stock_availability']) ||  $data['stock_availability']=='AVAIL'){
				$this->db->having('availableQty>0');
			}else if(!empty($data['stock_availability']) &&  $data['stock_availability']=='NOTAVAIL'){
				$this->db->having('availableQty=0');
			}
	...
	public function getStockInStoreBase($data){		
	    $this->db->select('SUM(ss.qty) as total_qty, (SELECT SUM(qty) FROM svi_challan_product cp LEFT JOIN svi_challan sc ON (cp.challan_id=sc.challan_id) LEFT JOIN svi_batch sbt ON (cp.batch_id=sbt.batch_id)  WHERE sc.is_deleted="N" AND sbt.exp_date>=NOW() AND cp.product_id = ss.product_id AND cp.store_id = ss.store_id) as challan_qty, (SELECT SUM(return_qty) FROM svi_challan_product cp LEFT JOIN svi_challan sc ON (cp.challan_id=sc.challan_id) LEFT JOIN svi_batch sbt ON (cp.batch_id=sbt.batch_id) WHERE sc.is_deleted="N" AND sbt.exp_date>=NOW() AND cp.product_id = ss.product_id AND cp.store_id = ss.store_id ) as return_qty')
            ->from('stock ss,batch sb');	

		$array = array('ss.product_id' => $data['product_id'], 'ss.store_id' => $data['store_id'], 'ss.status' => 'A');
		$this->db->where($array);
		$this->db->where("sb.batch_id=ss.batch_id");
		$this->db->where("sb.exp_date>=NOW()");
		$this->db->group_by('ss.product_id');
		$query = $this->db->get();
	...
	public function getProductExpiryYear($data){
		$this->db->select('pack_unit,unit_name,category_id,gst, (SELECT expiry_year FROM svi_category WHERE category_id = svi_product.category_id) as expiry_year')
            ->from('product')
	...
	public function getStockListDownload($data) {
