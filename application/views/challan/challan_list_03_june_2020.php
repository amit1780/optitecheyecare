		  ...
		  <tbody>
			<?php if(($challanInfo)){ ?>
				<?php foreach($challanInfo as $challan){ ?>				
					<tr class="<?php if($challan['is_deleted'] == 'Y'){ echo "disabled rowcolor"; } ?>">						
						...
						<td><?php if(!empty($challan['ord_id'])){ ?><a target="_blank" href="<?php echo site_url('orderView'); ?>/<?php echo $challan['order_id']; ?>" ><?php echo getOrderNo($challan['order_id']); ?></a><?php } else { echo "Deleted"; } ?></td>
						...
						<td><?php if(!empty($challan['ord_id'])){ ?><i class="<?php echo $challan['currency_faclass']; ?>" style="font-size:13px;"></i>&nbsp;<?php echo number_format((float)($challan['net_total']), 2, '.', ''); } ?></td>
						...
						<td class="">
							...
							<?php } else { ?>
								<a style="padding-left:7px;" href="#" onClick="viewDeleteReason(<?php echo $challan['challan_id']; ?>,<?php echo $challan['corder_id']; ?>);" title="View Deleted Reason" ><i class="fas fa-eye"></i></a> 
							<?php } ?>
