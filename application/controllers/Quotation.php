...
class Quotation extends CI_Controller {
	...
	public function index()
	{
		...
		foreach($data['quotations'] as $quotation){
			...
			#Added on 24-Dec-2021 to get country ID
			#$customerCountry=$this->db->get_where('customer',array('customer_id'=>$quotation['customer_id']))->country_id;
			...
			if($quotation['currency_id'] == 1){
				...
				foreach($productgst as $progst){
				    #Added by Arun 30-dec-2020 Start
				    $perProFrchWthGst=0;
					if($freight_gst==0){
	...
	public function generateQuote() {
		...
		if ($this->form_validation->run() == false) {
			...
			if($data['certificate_id']){
				$this->quotation_model->addProductsCertificate($data,$quotation_id);
			}
	...
	public function editQuotation($quotation_id) {
		...
		$quotationCertificateInfo = $this->quotation_model->getQuotationCertificateById($quotation_id);
		$certificateArr=array();
		$i=0;
		foreach($quotationCertificateInfo as $pcInfo){
...
public function savePdf() {
	...
	$footer = '<div style="margin-right: 25px;margin-left: 25px;font-size:12px;width:100%;text-align:right;"><span>Page - {PAGENO}</span></div>';
       ...
       25, // margin bottom
		...
		$certificateFile=array();
		$quotationCertificates = $this->quotation_model->getQuotationCertificateById($quotation_id);
		$j=0;
		foreach($quotationCertificates as $productCertificate){
			if($productCertificate['certificate_path_file']){
				$j++;
				$json[] = array(
					'quote_file' 		=> $productCertificate['certificate_path_file'],						
					'quote_file_name' 	=> $productCertificate['certificate_name'],
					'person' 			=> $data['customerInfo']->person_title ." ". $data['customerInfo']->contact_person,
					'email_to' 			=> $data['customerInfo']->email
				);
			}
		...
		$json['product_count'] = $i;
		$json['certificate_count'] = $j;
	...
	public function quotationProductFile() {
		$json=array();
		$data['quoteProductInfo'] = $this->quotation_model->getQuotationProductById($this->input->post("quotation_id"));	
		foreach($data['quoteProductInfo'] as $quoteProduct){
			$productPdf = $this->product_model->getProductById($quoteProduct['prod_id']);
	...
	public function quotationCertificateFile() {
		$json=array();
		$quotationCertificates = $this->quotation_model->getQuotationCertificateById($this->input->post("quotation_id"));
		
		foreach($quotationCertificates as $productCertificate){
			if($productCertificate['certificate_path_file']){
				
				$json[] = array(
					'file_url' 		=> $productCertificate['certificate_path_file'],						
					'file_name' 	=> $productCertificate['certificate_name']
				);
			}
		}
		echo json_encode($json);						
