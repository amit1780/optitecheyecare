		...
		<div class="row">
			...
			<div class="col-sm-6">
				<div class="float-right">
					...
					<a href="<?php echo site_url('stock/stockListDownload')."?".$queryString; ?>" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Download"><i class="fas fa-download"></i></a> &nbsp; 
	...
	<div class="card-body">
		...
		<form role="form" class="needs-validation" id="filterForm" data-toggle="validator" method="get" action="<?php echo site_url($form_action);?>" enctype="multipart/form-data" novalidate>	
			<div class="row">														
				...
				<div class="col-sm-2">
				  <div class="form-group">
					<div class="control-label">Show Stocks</div> 
					<select name="is_expired" id="is_expired" class="form-control" required>
						<option value="N" <?php if($is_expired=='N'){ echo "selected"; } ?>>Unexpired Only</option>
						<option value="Y" <?php if($is_expired=='Y'){ echo "selected"; } ?>>Expired Only</option>
						<option value="A" <?php if($is_expired=='A'){ echo "selected"; } ?>>All</option>
				...
				<div class="col-sm-2">
				  <div class="form-group">
					<div class="control-label">Stock Availability</div> 
					<select name="stock_availability" id="stock_availability" class="form-control" required>
						<option value="ALL" <?php if($stock_availability=='ALL'){ echo "selected"; } ?>>All</option>
						<option value="AVAIL" <?php if($stock_availability=='AVAIL'){ echo "selected"; } ?>>Only Available</option>
						<option value="NOTAVAIL" <?php if($stock_availability=='NOTAVAIL'){ echo "selected"; } ?>>Not Available</option>
			...
			<?php if(($stocks)){ ?>
				<?php foreach($stocks as $stock){ ?>
					  ...
					  <tr>
						...
						<td><?php echo ($stock['availableQty']); ?></td>
...
<script> 
 $(document).ready(function(){  
	...
	$('#create_excel').click(function(){  
		...
		var urlll = base_url +"/downloadExcelFile?"+qry_string;		
