					...
					<table id="order-list" class=" table order-list">
						...
						<tbody> -->
							<?php 
								...
								$condition=Array();
								$condition['product_id']=$productInfo['prod_id'];
								$condition['currency_id']=$quotationInfo->currency_id;
								$minSellingPrice=$ci->quotation_model->getMinPrice($condition);
								...
								?>								
									<tr class='prolist'>
										...
										<td ><input type="text" autocomplete="off" class="form-control proqty input-class-price" id="proqty_<?php echo $productInfo['id']; ?>" name="qty[]" value="<?php echo $productInfo['qty']; ?>"/></td>
										
										<td ><input type="text" autocomplete="off" class="form-control proprs price-box input-class-price" id="proprs_<?php echo $productInfo['id']; ?>" name="price[]" value="<?php echo number_format((float)$withGstPrice, 2, '.', ''); ?>" readonly/></td>
										...
										&nbsp;&nbsp; <input style="width:40%;float:left;margin-left:10px;" type="text" class="form-control prodscrate input-class-price" id="prodscrate_<?php echo $productInfo['id']; ?>" name="discount_rate[]" value="<?php echo number_format((float)$productInfo['discount_per'], 4, '.', ''); ?>" /><span style="vertical-align: -moz-middle-with-baseline;margin-left: -10px;">%</span></td>
										...
										<td >
										<input type="text" class="form-control protot" id="protot_<?php echo $productInfo['id']; ?>" name="total[]" value="<?php echo number_format((float)$netAmount, 2, '.', ''); ?>" readonly />
										<input type="hidden" class="form-control protot" id="selling_price_min_<?php echo $productInfo['id']; ?>" name="selling_price_min[]" value="<?php echo $minSellingPrice->min_price; ?>" readonly />
										</td>
					...
					<div class="row"  >
						...
						<div class="col-sm-2">						  
						  <div class="form-group">							
							...
							<input type="text" name="qty" value="" id="qty" class="form-control input-class-price" autocomplete='off' required>		
						  </div>
						</div>	
						<div class="col-sm-2">						  
						  <div class="form-group">							
							<div class="control-label" >Rate (Per unit with gst)</div> 
							<input type="text" name="price" value="" id="price" class="form-control price-box input-class-price" autocomplete='off' <?php if($this->session->userdata('group_type') != 'SADMIN'){ echo ' readonly '; }?> required>
						...
						<div class="col-sm-2">						  
						  <div class="form-group">							
							...
							<input type="text" name="discount" value="" id="discount" class="form-control input-class-price" autocomplete='off' >
	...
	<script>
		$(document).ready(function() {
			...
			$("#addrow").on("click", function () {				
				...
				var selling_price=parseFloat(price)-parseFloat(discount);
				if( parseFloat(selling_price)!=0){
    				if( parseFloat(selling_price) < parseFloat(min_price)){
    					alert("Product can't be sold below the price "+min_price);
    					return false;
    				}
				}
			...
			$(".prodscrate").keyup(function(){				
				...
				var qty = $("#proqty_"+pro_quote_id).val();
				var price = $("#proprs_"+pro_quote_id).val();
				var minsellingPrice = $("#selling_price_min_"+pro_quote_id).val();
				var min_price=minsellingPrice;
				minsellingPrice=parseFloat(minsellingPrice)*parseInt(qty);
				...
				if(discount_type == 'per'){	
					...
					if(ftotal !=0 && ftotal<minsellingPrice){
						alert("Product can't be sold below the price "+min_price);
						$(':input[type="submit"]').prop('disabled', true);
						//$("#scraping_message").removeClass('text-success');	
						$("#prodscrate_"+pro_quote_id).addClass('border border-danger font-weight-bold');
					}else{
						$(':input[type="submit"]').prop('disabled', false);
						$("#prodscrate_"+pro_quote_id).removeClass('border border-danger font-weight-bold');
						$("#prodsc_"+pro_quote_id).removeClass('border border-danger font-weight-bold');
					}
				}				
			});
			...
			$(".prodsc").keyup(function(){				
				...
				if(discount_type == 'amt'){	
					...
					ftotal = parseInt(qty) * parseFloat(ftotal);
					$("#prodscrate_"+pro_quote_id).val(per.toFixed(2));
					$("#protot_"+pro_quote_id).val(ftotal.toFixed(2));
					...
					if(ftotal !=0 && ftotal<minsellingPrice){
						alert("Product can't be sold below the price "+min_price);
						$(':input[type="submit"]').prop('disabled', true);
						$("#prodsc_"+pro_quote_id).addClass('border border-danger font-weight-bold');
					}else{
						$(':input[type="submit"]').prop('disabled', false);
						$("#prodsc_"+pro_quote_id).removeClass('border border-danger font-weight-bold');
						$("#prodscrate_"+pro_quote_id).removeClass('border border-danger font-weight-bold');
...
<script>
$(document).ready(function(){		
	$(".input-class-price").on('keyup', function() {
