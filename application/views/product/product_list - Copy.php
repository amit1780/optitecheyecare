				...
				<?php if(($products)){ ?>
					<?php foreach($products as $product){ ?>				
						<tr>
							...
							<td class="text-center"><a href="<?php echo site_url('productView'); ?>/<?php echo $product['product_id']; ?>"><i class="fas fa-eye"></i></a> &nbsp;&nbsp;&nbsp;&nbsp; <a href="<?php echo site_url('editProduct'); ?>/<?php echo $product['product_id']; ?>" data-toggle="tooltip" data-placement="top" ><i class="far fa-edit"></i></a></td>							
						</tr>
					<?php } ?>  
				<?php } else { ?>
					<tr>
	...
	$(document).ready(function(){
		...
		$('#product_name').typeahead({			
			source: function (query, result) {				
				$.ajax({
					url:'<?php echo site_url('getProductName2'); ?>',
					data: 'name=' + query,            
					dataType: "json",
					type: "POST",
		...
		$('#product_name').on('autocompletechange change', function () {
			...
			$.ajax({
				url:'<?php echo site_url('getProductModel2');?>',
				method: 'post',
