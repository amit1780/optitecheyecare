...
class Quotation_model extends CI_Model {
	...
	public function addProductsCertificate($data,$quotation_id){
		$this->db->select('*');
		$this->db->from('quote_certificate');
		$this->db->where('quotation_id',$quotation_id);					
		$query3 = $this->db->get()->row();
		if($query3->quotation_id){
			$this->db->where('quotation_id', $quotation_id);
			$this->db->delete('quote_certificate');
			
			foreach($data['certificate_id'] as $certificate_id){
				$certificate = array(				
					'quotation_id'   		=> $quotation_id,
					'certificate_id'   		=> $certificate_id				
				);	
				$this->db->insert('quote_certificate', $certificate);
			}
		}else{
			foreach($data['certificate_id'] as $certificate_id){
				$certificate = array(				
	...
	public function getQuotationCertificateById($quotation_id){
		$this->db->select('*,(SELECT path FROM svi_certificates WHERE certificate_id = svi_quote_certificate.certificate_id) as certificate_path_file, (SELECT certificate_name FROM svi_certificates WHERE certificate_id = svi_quote_certificate.certificate_id) as certificate_name');
		$this->db->from('quote_certificate');
		$this->db->where('quotation_id', $quotation_id);
		$query = $this->db->get();
	...
	public function getQuoteCustomerList($limit, $start,$data) {
		...
		if(!empty($data['quote_id'])){
			$qryArr['quote_customer.id']= trim(financialQuoteId($data['quote_id'],$data['fqyear']));
		}
	...
	public function getCompleteQuoteCustomerList($limit, $start,$data) {
		...
		if(!empty($data['order_id'])){
			$qryArr['quote_customer.order_id']= trim(financialOrderId($data['order_id'],$data['foyear']));
		}
	...
	public function getQuotationCustomerById($quotation_id){
		...
		$this->db->select('*, quote_customer.id as quote_id, (SELECT username FROM svi_user WHERE user_id = svi_quote_customer.created_by) as created_user');
		$this->db->from('quote_customer');
